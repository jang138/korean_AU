...
def load_data_from_hf(dataset_name, split):
    """HuggingFace에서 데이터셋 로드 → pandas DataFrame 반환"""
    print(f"=== {split} 데이터 로딩 시작 ===")
...
def test_hf_dataset_loading():
    ...
    dataset_name = "onestone11/nikl-hate-speech"  # 본인 데이터셋 이름으로 변경
    
    print("HuggingFace 데이터셋 로딩 테스트 시작")
    print("=" * 80)
    ...
    for split in splits:
        print(f"\n{split.upper()} 데이터 테스트")
        df = load_data_from_hf(dataset_name, split)
    ...
    for split, result in results.items():
        if result['success']:
            print(f"[OK] {split:12s}: {result['shape'][0]:5d} rows × {result['shape'][1]} cols")
        else:
